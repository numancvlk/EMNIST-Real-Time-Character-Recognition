...
trainedModel.load_state_dict(torch.load("myEMNISTMODEL.pth", map_location=device))
...
while True:
    ...
    thresh = cv.morphologyEx(
        cv.threshold(cv.GaussianBlur(grayFrame,(5,5),0), 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU),
        cv.MORPH_OPEN,
    ...
    if contours:
        ...
        if 2000 < cv.contourArea(cnt) < 80000:
        ...
        # h → Dikdörtgenin yüksekliği (height)
            ...
            if predConfidence >= confidenceThreshold:
                ...
                cv.putText(frame,f"Pred = {stablePrediction} ({predConfidence*100:.0f}%)", (x,y-10), cv.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
